...
export default function AdminLayout({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-screen bg-mist/30">
      {/* Sidebar */}
      <aside className="w-64 bg-forest text-sand flex-shrink-0 fixed h-full flex flex-col">
        <div className="p-6">
          <Link href="/admin" className="flex flex-col">
            <span className="font-serif text-2xl font-bold tracking-tight">TALAL</span>
            <span className="text-[10px] tracking-[0.2em] uppercase text-gold font-semibold">Admin Panel</span>
          </Link>
        </div>
        ...
        <div className="p-4 mt-auto">
          <Link href="/" className="flex items-center space-x-3 px-4 py-3 rounded-sm text-sand/70 hover:text-sand hover:bg-white/10 transition-colors">
